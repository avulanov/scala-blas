...
val range = for (b  <- 0 to 6) yield { math.pow(2, b).toInt}
// Float
for (i <- range) { val a = breeze.linalg.DenseMatrix.rand[Double](i, i).mapValues(_.toFloat); val b = breeze.linalg.DenseMatrix.rand[Double](i, i).mapValues(_.toFloat); System.gc(); val t = System.nanoTime(); val c = a * b; println(c(0,0) + " " + i + "x" + i + "*" + i + "x" + i + "\t" + (System.nanoTime() - t) / 1e9)}
// Double
for (i <- range) { val a = breeze.linalg.DenseMatrix.rand[Double](i, i); val b = breeze.linalg.DenseMatrix.rand[Double](i, i); System.gc(); val t = System.nanoTime(); val c = a * b; println(i + "x" + i + "*" + i + "x" + i + "\t" + (System.nanoTime() - t) / 1e9)}